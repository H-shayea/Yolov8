...
class Split : public Node {
	...
	virtual void resolve(void) override
	{
		...
		if(num_inputs < 2) {
			...
			// Create a virtual split tensor based on the number of outputs needed
			// This assumes equal splits
			LOG(INFO) << "Split node with single input - using equal splits";
			name_input(0, "input");
			
			// For YOLOv8, we need to determine number of outputs from the graph
			// For now, assume 2 outputs (most common case)
			int64_t num_outputs = 2; // This could be made smarter
